...
# Generation Time: 2018-03-27 02:32:45 +0000
# ************************************************************
...
# Dump of table dietclientplans
# ------------------------------------------------------------

DROP TABLE IF EXISTS `dietclientplans`;

CREATE TABLE `dietclientplans` (
...
# Dump of table dietitianclients
# ------------------------------------------------------------

DROP TABLE IF EXISTS `dietitianclients`;

CREATE TABLE `dietitianclients` (
  `dietitianid` int(11) NOT NULL,
  `userid` bigint(11) NOT NULL,
  PRIMARY KEY (`userid`,`dietitianid`),
  KEY `dietitianid` (`dietitianid`),
  CONSTRAINT `dietitianclients_ibfk_1` FOREIGN KEY (`dietitianid`) REFERENCES `dietitians` (`dietitianid`),
  CONSTRAINT `userForeign` FOREIGN KEY (`userid`) REFERENCES `users` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;



# Dump of table dietitianpractices
# ------------------------------------------------------------

DROP TABLE IF EXISTS `dietitianpractices`;

CREATE TABLE `dietitianpractices` (
...
CREATE TABLE `dietitians` (
  ...
  KEY `dietitians_user_foreign` (`userid`),
  CONSTRAINT `dietitians_ibfk_1` FOREIGN KEY (`practiceid`) REFERENCES `practices` (`practiceid`) ON DELETE CASCADE,
  CONSTRAINT `dietitians_user_foreign` FOREIGN KEY (`userid`) REFERENCES `users` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

LOCK TABLES `dietitians` WRITE;
/*!40000 ALTER TABLE `dietitians` DISABLE KEYS */;

INSERT INTO `dietitians` (`dietitianid`, `firstname`, `lastname`, `practiceid`, `userid`)
VALUES
	(1,'John','Smith',1,1);

/*!40000 ALTER TABLE `dietitians` ENABLE KEYS */;
UNLOCK TABLES;
...
INSERT INTO `practices` (`practiceid`, `name`, `suburb`, `postcode`, `number`, `address`)
VALUES
	(1,'Health You','Suburb 3',1223,1,'Little Street');

/*!40000 ALTER TABLE `practices` ENABLE KEYS */;
UNLOCK TABLES;


# Dump of table savedmeals
# ------------------------------------------------------------

DROP TABLE IF EXISTS `savedmeals`;
...
# Dump of table userfridges
# ------------------------------------------------------------

DROP TABLE IF EXISTS `userfridges`;

CREATE TABLE `userfridges` (
...
VALUES
	(1,'mysqltest@email.com','$2a$11$YaujfQ9m5XPQAFvLPxRAgux7dt4RkvPXu79H2JJfMdNp93bHpgn3S',NULL,NULL,NULL,9,'2018-03-26 08:46:00','2018-03-26 08:44:00','::1','::1','2018-03-25 13:13:35','2018-03-27 02:03:01','mysqlworking','123','123456',1,1,1,0);
